...
try {
    ...
    // Prepare statements for rooms deletion and enabling
    ...
    while ($rooms = $roomResult->fetch_assoc()) {
        // Delete rooms from booking
        $deleteRoom->bind_param("ii", $bookingId, $rooms['room_id']);
        if (!$deleteRoom->execute()) {
            throw new Exception('Failed to cancel rooms');
        }

        // Enable the rooms
        $enableRoom->bind_param("i", $rooms['room_id']);
        if (!$enableRoom->execute()) {
            throw new Exception('Failed to enable rooms');
        }
    }
